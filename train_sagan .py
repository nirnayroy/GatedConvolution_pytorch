...
from data.inpaint_dataset import CMB_Dataset
...
def validate(netG, netD, GANLoss, ReconLoss, DLoss, optG, optD, dataloader, epoch, device=device, batch_n="whole"):
    """
    validate phase
    """
    netG.to(device)
...
def train(netG, netD, GANLoss, ReconLoss, DLoss, optG, optD, dataloader, epoch, device=device, val_datas=None):
    """
...
def main():
    ...
    '''
    train_loader = CMB_Dataset(batch_size=16).loader()
    '''
    ...
    for i in range(epoch):
        ...
        #train data
        train(netG, netD, gan_loss, recon_loss, dis_loss, optG, optD, train_loader, i, device=device)
        ...
        torch.save(saved_model, 'epoch_'+str(i)+'_ckpt.pth')
        
        print('epoch:', i)
    torch.save(saved_model, 'latest_ckpt.pth')
